...
export const appConfig = {
  ...
  imageUpload: {
    ...
    allowedMimeTypes: (process.env.IMAGE_ALLOWED_TYPES || 'image/jpeg,image/png,image/webp,image/gif').split(','),
  },
  
  image: {
    maxSizeMB: parseInt(process.env.IMAGE_MAX_SIZE_MB || '10', 10),
    allowedTypes: (process.env.IMAGE_ALLOWED_TYPES || 'image/jpeg,image/png,image/webp,image/gif').split(','),
    sizes: {
