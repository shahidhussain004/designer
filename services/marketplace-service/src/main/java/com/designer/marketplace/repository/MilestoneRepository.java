...
public interface MilestoneRepository extends JpaRepository<Milestone, Long> {
    ...
    @Query("SELECT m FROM Milestone m WHERE m.project.id = :projectId AND m.status IN :statuses ORDER BY m.sequenceOrder ASC")
    ...
    @Query("SELECT SUM(m.amount) FROM Milestone m WHERE m.project.id = :projectId")
