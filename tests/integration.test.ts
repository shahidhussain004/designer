...
describe('Designer Marketplace API - Integration Tests', () => {
  ...
  describe('Dashboard Statistics', () => {
    ...
    test('Should get FREELANCER dashboard stats', async () => {
      ...
      expect(response.data).toHaveProperty('proposalsSubmitted');
      expect(response.data).toHaveProperty('proposalsAccepted');
      expect(response.data).toHaveProperty('totalEarnings');
      expect(response.data).toHaveProperty('avgRating');
      expect(typeof response.data.proposalsSubmitted).toBe('number');
    });
  });
  ...
  describe('Performance', () => {
    test('Should load 100+ jobs under 1 second', async () => {
  ...
  describe('Complete E2E Workflow', () => {
    test('Should complete full marketplace workflow', async () => {
      ...
      // 7. Check dashboards
      const clientDash = await api.get('/dashboards/company', getAuthHeader(clientUser.token!));
