...
const tagBadgeColors: Record<string, string> = {
  ...
  Partnership: 'bg-warning-100 text-warning-700',
...
export default function PressPage() {
  return (
    <PageLayout>
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-20 lg:py-28">
          <div className="max-w-2xl">
            ...
            <div className="flex flex-wrap gap-4">
              <a href="#press-kit" className="inline-flex items-center gap-2 px-5 py-3 bg-white text-secondary-900 font-semibold rounded-lg hover:bg-secondary-100 transition-colors text-sm">
        ...
        <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-12 flex-wrap gap-4">
            <div>
              ...
              <h2 className="text-3xl font-bold text-secondary-900 mt-2">Latest announcements</h2>
            </div>
          ...
          <div className="space-y-6">
            {pressReleases.map((pr) => (
              <article key={pr.title} className="bg-secondary-50 rounded-2xl border border-secondary-200 p-6 hover:border-primary-200 hover:shadow-sm transition-all duration-200 group cursor-pointer">
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-3">
                      <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${tagBadgeColors[pr.tag] || 'bg-secondary-100 text-secondary-600'}`}>
                        {pr.tag}
                      </span>
                      <span className="text-secondary-400 text-xs">{pr.date}</span>
                      <span className="text-secondary-400 text-xs">· {pr.readTime}</span>
                    </div>
                    <h3 className="font-bold text-secondary-900 text-lg leading-snug mb-2 group-hover:text-primary-700 transition-colors">
                      {pr.title}
                    </h3>
                    <p className="text-secondary-500 text-sm leading-relaxed">{pr.excerpt}</p>
                  </div>
                  <div className="flex-shrink-0 self-center">
                    <svg className="w-5 h-5 text-secondary-400 group-hover:text-primary-600 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      <section className="bg-secondary-50 py-20 lg:py-28">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            ...
            <h2 className="text-3xl font-bold text-secondary-900 mt-3 mb-3">What people are saying</h2>
            <p className="text-secondary-500 max-w-xl mx-auto">Selected coverage from design, technology, and business publications.</p>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {coverage.map((item) => (
              <div key={item.headline} className="bg-white rounded-2xl border border-secondary-200 p-6 hover:border-secondary-300 hover:shadow-sm transition-all duration-200">
                <div className="flex items-center justify-between mb-4">
                  <span className="font-bold text-secondary-900 text-sm">{item.publication}</span>
                  <span className="text-secondary-400 text-xs">{item.year}</span>
                </div>
                <span className="inline-block text-xs bg-secondary-100 text-secondary-500 rounded-full px-2.5 py-0.5 mb-3">{item.category}</span>
                <blockquote className="text-secondary-700 text-sm leading-relaxed italic">{item.headline}</blockquote>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-white py-16 lg:py-20">
        <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <h2 className="text-3xl font-bold text-secondary-900 mt-3">Awards & accolades</h2>
          </div>
          <div className="divide-y divide-secondary-100">
            {awards.map((a) => (
              <div key={a.award} className="flex items-center justify-between py-5">
                <div className="flex items-center gap-5">
                  <span className="text-sm font-mono text-secondary-400 w-12 flex-shrink-0">{a.year}</span>
                  <div>
                    <p className="font-semibold text-secondary-900">{a.award}</p>
                    <p className="text-secondary-400 text-sm">{a.org}</p>
                  </div>
                </div>
                <svg className="w-5 h-5 text-warning-400 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
      ...
      {/* ── Press Kit ── */}
      <section id="press-kit" className="bg-secondary-900 text-white py-20">
        <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={item} className="flex items-center gap-3 text-secondary-300 text-sm">
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-8">
              ...
              <div className="space-y-4 mb-8">
                <div>
                  <p className="text-secondary-400 text-xs mb-1 uppercase tracking-wide">Press contact</p>
                ...
                <div>
                  <p className="text-secondary-400 text-xs mb-1 uppercase tracking-wide">Response time</p>
                  <p className="text-secondary-300 text-sm">Typically within 4 business hours</p>
                </div>
              </div>
              <a
