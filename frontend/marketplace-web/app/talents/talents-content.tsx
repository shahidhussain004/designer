          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
            ...
            <div className="relative lg:col-span-1">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 border border-secondary-300 rounded-lg focus:outline-none"
              />
            </div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full px-4 py-2.5 border border-secondary-300 rounded-lg focus:outline-none"
              />
            </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-6">
            <p className="text-secondary-600">
              Showing <span className="font-semibold text-secondary-900">{filteredFreelancers.length}</span> talent
          ...
          {!isError && filteredFreelancers.length === 0 ? (
            <div className="bg-white rounded-xl border border-secondary-200 p-12 text-center">
              ...
              <h3 className="text-xl font-semibold text-secondary-900 mb-2">No freelancers found</h3>
              <p className="text-secondary-600 mb-6">Try adjusting your filters</p>
              <button
                onClick={clearFilters}
            ...
            <>
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-start justify-between gap-4 mb-2">
                              <div>
                                ...
                                <p className="text-sm text-secondary-500">@{freelancer.username}</p>
                              </div>
                              <div className="text-right flex-shrink-0">
                                {freelancer.hourlyRate ? (
                                  <div>
                                    <span className="text-lg font-bold text-secondary-900">${freelancer.hourlyRate}</span>
                                    <span className="text-secondary-500">/hr</span>
                                  </div>
                                ) : (
                                  <span className="text-secondary-400 text-sm">Rate not set</span>
                                )}
                              </div>
                            </div>
                            ...
                            <div className="flex items-center gap-4 mb-3">
                              ...
                              {typeof freelancer.ratingAvg !== 'undefined' && (
                                <div className="flex items-center text-sm">
                                  <svg className="w-4 h-4 text-warning-400 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                                  </svg>
                                  <span className="font-medium text-secondary-900">{freelancer.ratingAvg.toFixed(1)}</span>
                                  <span className="text-secondary-400 ml-1">({freelancer.ratingCount})</span>
                                </div>
                              )}
                            </div>
                            ...
                            <div className="flex items-center justify-between">
                              {freelancer.skills && freelancer.skills.length > 0 && (
                                <div className="flex flex-wrap gap-1.5">
                                  {freelancer.skills.slice(0, 5).map((skill, idx) => (
                                    <span
                                      key={idx}
                                      className="px-2 py-0.5 bg-secondary-100 text-secondary-600 rounded text-xs font-medium"
                                    >
                                      {skill}
                                    </span>
                                  ))}
                              ...
                              {typeof freelancer.completionRate !== 'undefined' && (
                                <span className="text-sm text-secondary-500">
                                  <span className="font-medium text-success-600">{freelancer.completionRate}%</span> Job Success
                                </span>
                              )}
                            </div>
                          </div>
              ...
              {layoutMode === 'grid' && (
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredFreelancers.map(freelancer => (
                    <Link key={freelancer.id} href={`/freelancers/${freelancer.id}`} className="block group">
                      <div className="bg-white rounded-xl border border-secondary-200 p-6 hover:border-primary-300 hover:shadow-lg transition-all h-full flex flex-col">
                        {/* Header with Avatar */}
                        ...
                        {/* Rate & Rating */}
                        <div className="flex items-center justify-between mb-4 pb-4 border-b border-secondary-100">
                          {freelancer.hourlyRate ? (
                            <div>
                              <span className="text-xl font-bold text-secondary-900">${freelancer.hourlyRate}</span>
                              <span className="text-secondary-500">/hr</span>
                            </div>
                          ) : (
                            <span className="text-secondary-400 text-sm">Rate not set</span>
              ...
              <div className="flex items-center justify-between gap-4 flex-wrap">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 px-3 py-2 border border-secondary-300 rounded-lg text-sm font-medium text-secondary-700 hover:bg-secondary-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <button
                          ...
                          className={`w-10 h-10 rounded-lg font-medium transition-colors ${
                            idx === currentPage
                              ...
                              : 'border border-secondary-300 text-secondary-700 hover:bg-secondary-50 disabled:opacity-50 disabled:cursor-not-allowed'
                          }`}
                        >
                          {idx + 1}
                        </button>
                ...
                <div className="flex items-center gap-2">
                  <label className="text-sm text-secondary-500">Show</label>
                  <select
                    value={itemsPerPage}
                    onChange={(e) => { setItemsPerPage(parseInt(e.target.value)); setCurrentPage(0); }}
                    className="px-2 py-1 border border-secondary-300 rounded-lg text-sm"
                  >
