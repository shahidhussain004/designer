          ...
          <div className="text-center">
            {error ? (
              ...
              <>
                ...
                <p className="text-secondary-600 mb-4">This course does not exist</p>
      ...
      <div className="bg-secondary-900 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <nav className="flex items-center gap-2 text-sm text-secondary-400 mb-6">
            ...
            <span>/</span>
            <span className="text-secondary-300">{course.category}</span>
          </nav>

          {/* Title and Description */}
          <h1 className="text-3xl lg:text-4xl font-bold mb-4">{course.title}</h1>
          <p className="text-lg text-secondary-300 mb-6 max-w-3xl">{course.description}</p>
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm">
            ...
            <span className="text-secondary-500">•</span>
            <span className="px-2 py-1 bg-secondary-800 rounded text-xs font-medium">{course.skillLevel}</span>
          </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {Array.isArray(course.learningOutcomes) && course.learningOutcomes.length > 0 && (
              <div className="bg-white rounded-lg shadow-sm border border-secondary-200 p-6">
                <h2 className="text-xl font-bold text-secondary-900 mb-4">What You&apos;ll Learn</h2>
                ...
                {displayedLessons.map((lesson: Lesson, index: number) => (
                  <div key={lesson.id} className="py-4 flex items-center justify-between">
                    <div className="flex items-start gap-4 flex-1">
                      <span className="text-sm font-medium text-secondary-400 w-8">{index + 1}</span>
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-3 mt-1 text-sm text-secondary-500">
                          ...
                          {lesson.isPreview && (
                            <span className="px-2 py-0.5 bg-primary-100 text-primary-700 rounded text-xs font-medium">Preview</span>
                          )}
                        </div>
                      </div>
                    </div>
            ...
            {Array.isArray(course.requirements) && course.requirements.length > 0 && (
              <div className="bg-white rounded-lg shadow-sm border border-secondary-200 p-6">
                <h2 className="text-xl font-bold text-secondary-900 mb-4">Prerequisites & Requirements</h2>
            ...
            {Array.isArray(course.tags) && course.tags.length > 0 && (
              <div className="bg-white rounded-lg shadow-sm border border-secondary-200 p-6">
                <h2 className="text-xl font-bold text-secondary-900 mb-4">Skills You&apos;ll Master</h2>
            ...
            <div className="sticky top-6">
              <div className="bg-white rounded-lg shadow-sm border border-secondary-200 overflow-hidden">
                ...
                <div className="relative aspect-video bg-secondary-100">
                  {course.thumbnailUrl ? (
                    ...
                    <div className="flex items-center justify-center h-full">
                      <PlayCircle className="w-16 h-16 text-secondary-300" />
                    </div>
                  )}
                </div>
                ...
                <div className="p-6 space-y-4">
                  ...
                  {/* Guarantee */}
                  {course.price > 0 && (
                    <div className="flex items-center gap-2 p-3 bg-secondary-50 rounded-lg text-sm text-secondary-600">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center gap-3 text-sm">
                      ...
                      <div>
                        <p className="text-secondary-500">Total Duration</p>
                        <p className="font-medium text-secondary-900">{totalHours}h {totalMinutes}m</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3 text-sm">
                      <BookOpen className="w-5 h-5 text-secondary-400" />
                      <div>
                        <p className="text-secondary-500">Lessons</p>
                        <p className="font-medium text-secondary-900">{course.lessonsCount} lessons</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3 text-sm">
                      ...
                      <div>
                        ...
                        <p className="font-medium text-secondary-900">Completion Certificate</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3 text-sm">
                      <Globe className="w-5 h-5 text-secondary-400" />
                      <div>
                        <p className="text-secondary-500">Access</p>
                        <p className="font-medium text-secondary-900">Lifetime Access</p>
                      </div>
                    </div>
                  </div>
                </div>
